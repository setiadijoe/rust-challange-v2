...
impl Frog {
    ...
    pub fn hop(&mut self) {
        ...
        info!("A frog hopped and the energy is {}", self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!("Warning the frog will go to sleep");
            self.sleep();
        }
    }
    pub fn sleep(&mut self) {
        if self.sleeping {
            ...
            error!("The frog has already sleeping");
        } else {
            self.sleeping = true;
        }
    }
...
impl Default for Frog {
    fn default() -> Self {
        ...
        trace!(target: "Default Frog" ,"A default frog has generated. {:?}", Frog {
            energy: 5,
            sleeping: false,
        });
        Frog {
